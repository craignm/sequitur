# Sequitur by Dr.Craig-Nevill Manning
# Ported from Java code by Dr,Eibe Frank
...
class Rule:
    ...
    def get_rules(self):
        ...
        while (processedRules<len(rules)):
            ...
            text=text+" "
            text=text+str(currentRule.count)
            text=text+'\tR'
            text=text+str(processedRules)+' -> '
            ...
            while True:
                ...
                if sym.is_nonterminal():
                    refered=sym.r
                    if len(rules)>refered.index and rules[refered.index]==refered:
                        index=refered.index
                    else:
                        index=len(rules)
                        refered.index=index
                        rules.append(refered)
                    text=text+'R'+str(index)
                else:
                    if sym.value == ' ':
                        text=text+'_'
                    elif sym.value == '\n':
...
class Symbol:
    ...
    def insert_after(self, to_insert):
        #print 'append ', to_insert.value, 'after ', self.value
...
def test():    
    assert run_sequitur('abracadabraabracadabra') == 'Usage\tRule\n 0\tR0 -> R1 R1 \n 2\tR1 -> R2 c a d R2 \n 2\tR2 -> a b r a \n'
